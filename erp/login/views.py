...
def home(request):
	...
	if(user.utype=='SUP'):
		if(request.method=='POST'):
			...
			if(order.status == 'New draft'):
				order.status='New SO'
			order.save()
			notify.send(sender=request.user, recipient=User.objects.all(), action_object=order, verb=user.username+' approved '+order.material+' on ' +date.today().strftime("%B %d, %Y")+ ' to status '+order.status)
		...
		return render(request,'support.html',{'user':user, 'data1': data1, 'data2':data2, 'data3':data3, 'notifications':request.user.notifications.all()})
	...
	if(user.utype=='OPE'):
		if(request.method=='POST'):
			...
			order.save()
			notify.send(sender=request.user, recipient=User.objects.all(), verb=user.username+' approved '+order.material+' on ' +date.today().strftime("%B %d, %Y")+ ' to status '+order.status)
		...
		return render(request,'operations.html',{'user':user, 'data1': data1, 'data2': data2, 'data3': data3, 'notifications':request.user.notifications.all()})
	...
	if(user.utype=='FIN'):
		...
		return render(request,'finance.html',{'user':user, 'data1': data1, 'data2': data2, 'data3': data3, 'data4':data4, 'notifications':request.user.notifications.all()})
