	...
	if(user.utype=='SUP'):
		if(request.method=='POST'):
			...
			if(order.status == 'New draft'):
				order.status='New SO'
			order.save()
			notify.send(sender=request.user, recipient=User.objects.all(), action_object=order, verb=user.username+' approved order '+str(order.id)+' on ' +date.today().strftime("%B %d, %Y")+ ' to status '+order.status)
